...
public class Cell implements Comparable<Cell> {
	...
	public enum Type {
		EMPTY, RESIDENCE, FACTORY, PARK, WATER, ROAD
	};
	...
	public Cell[] neighbors(int m) {
		int n = (i > 0 ? 1 : 0) + (i < m - 1 ? 1 : 0) + (j > 0 ? 1 : 0) + (j < m - 1 ? 1 : 0);
		Cell[] points = new Cell[n];
		if (i > 0)
			points[--n] = new Cell(i - 1, j);
		if (i < m - 1)
			points[--n] = new Cell(i + 1, j);
		if (j > 0)
			points[--n] = new Cell(i, j - 1);
		if (j < m - 1)
			points[--n] = new Cell(i, j + 1);
		return points;
	}
	...
	public static boolean isConnected(Set<Cell> points) {
		return isConnected(points, default_land_side);
	}

	public static boolean isConnected(Cell[] points) {
		return isConnected(points, default_land_side);
	}

	public static boolean isConnected(Set<Cell> points, int side) {
		return isConnected(points.toArray(new Cell[points.size()]), side);
	}
	...
	protected void buildWater() {
		buildType(Type.WATER);
	}

	protected void buildPark() {
		buildType(Type.PARK);
	}

	protected void buildResidence() {
